...
def get_sae_feature_activations(
    texts, model, tokenizer, ae, layer, device, feature_idx, batch_size=8
):
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc="Getting SAE activations"):
        ...
        feature_activations = (
            sae_features[:, :, feature_idx].mean(dim=1).cpu().numpy()
        )  # [batch]
...
def create_mixed_data(texts, labels, transform_fn, random_state=42):
    ...
    print(
        f"Mixed dataset: {num_to_transform}/{num_jailbreak} jailbreak samples transformed"
    )
...
def train_combined_classifier(
    train_data_orig, train_data_leetspeak, train_data_mixed=None
):
    ...
    X_leetspeak = np.column_stack(
        [
            ...
            train_data_leetspeak["feature_activations"].reshape(
                -1, 1
            ),  # SAE feature [N, 1]
        ]
    )
    ...
    y_combined = np.concatenate(
        [train_data_orig["labels"], train_data_leetspeak["labels"]]
    )
    ...
    if train_data_mixed is not None:
        X_mixed = np.column_stack(
            [
                ...
                train_data_mixed["feature_activations"].reshape(
                    -1, 1
                ),  # SAE feature [N, 1]
            ]
        )
...
def evaluate_classifiers(
    clf_combined, test_data_orig, test_data_leetspeak, test_data_mixed=None
):
    ...
    X_test_orig = np.column_stack(
        [test_data_orig["probs"], test_data_orig["feature_activations"].reshape(-1, 1)]
    )
    ...
    results["orig_model_leetspeak"] = {
        ...
        "auc": roc_auc_score(
            test_data_leetspeak["labels"], test_data_leetspeak["probs"][:, 2]
        ),
        "report": classification_report(
            test_data_leetspeak["labels"], orig_preds_leetspeak
        ),
    }
    ...
    results["combined_orig"] = {
        ...
        "auc": roc_auc_score(
            test_data_orig["labels"], clf_combined.predict_proba(X_test_orig)[:, 1]
        ),
    ...
    results["combined_leetspeak"] = {
        "accuracy": accuracy_score(
            test_data_leetspeak["labels"], combined_preds_leetspeak
        ),
        "auc": roc_auc_score(
            test_data_leetspeak["labels"],
            clf_combined.predict_proba(X_test_leetspeak)[:, 1],
        ),
        "report": classification_report(
            test_data_leetspeak["labels"], combined_preds_leetspeak
        ),
    ...
    if test_data_mixed is not None:
        ...
        results["orig_model_mixed"] = {
            ...
            "auc": roc_auc_score(
                test_data_mixed["labels"], test_data_mixed["probs"][:, 2]
            ),
            "report": classification_report(
                test_data_mixed["labels"], orig_preds_mixed
            ),
        }
        ...
        results["combined_mixed"] = {
            ...
            "auc": roc_auc_score(
                test_data_mixed["labels"],
                clf_combined.predict_proba(X_test_mixed)[:, 1],
            ),
            "report": classification_report(
                test_data_mixed["labels"], combined_preds_mixed
            ),
        }
...
def print_results(results):
    ...
    print("-" * 40)
    print(
        f"Original data - Accuracy: {results['orig_model_orig']['accuracy']:.4f}, AUC: {results['orig_model_orig']['auc']:.4f}"
    )
    print(
        f"Leetspeak data - Accuracy: {results['orig_model_leetspeak']['accuracy']:.4f}, AUC: {results['orig_model_leetspeak']['auc']:.4f}"
    )
    if "orig_model_mixed" in results:
        print(
            f"Mixed data - Accuracy: {results['orig_model_mixed']['accuracy']:.4f}, AUC: {results['orig_model_mixed']['auc']:.4f}"
        )

    print("\nCOMBINED CLASSIFIER:")
    print("-" * 40)
    print(
        f"Original data - Accuracy: {results['combined_orig']['accuracy']:.4f}, AUC: {results['combined_orig']['auc']:.4f}"
    )
    print(
        f"Leetspeak data - Accuracy: {results['combined_leetspeak']['accuracy']:.4f}, AUC: {results['combined_leetspeak']['auc']:.4f}"
    )
    if "combined_mixed" in results:
        print(
            f"Mixed data - Accuracy: {results['combined_mixed']['accuracy']:.4f}, AUC: {results['combined_mixed']['auc']:.4f}"
        )

    print("\nIMPROVEMENTS:")
    print("-" * 40)
    orig_improvement = (
        results["combined_orig"]["accuracy"] - results["orig_model_orig"]["accuracy"]
    )
    leetspeak_improvement = (
        results["combined_leetspeak"]["accuracy"]
        - results["orig_model_leetspeak"]["accuracy"]
    )
    ...
    if "combined_mixed" in results:
        mixed_improvement = (
            results["combined_mixed"]["accuracy"]
            - results["orig_model_mixed"]["accuracy"]
        )
...
def main():
    ...
    if FEATURE_IDX >= n_features:
        raise ValueError(
            f"Feature index {FEATURE_IDX} out of range. SAE has {n_features} features."
        )
    ...
    train_data_mixed = load_or_compute_data(
        "train", transform_fn=leet_speak, mixed=True
    )
    ...
    clf_combined = train_combined_classifier(
        train_data_orig, train_data_leetspeak, train_data_mixed
    )

    # Evaluate
    results = evaluate_classifiers(
        clf_combined, test_data_orig, test_data_leetspeak, test_data_mixed
    )
    ...
    results_file = os.path.join(
        CACHE_DIR, f"combined_classifier_results_layer{LAYER}_feature{FEATURE_IDX}.pkl"
    )
